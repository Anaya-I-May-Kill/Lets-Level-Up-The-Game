...
class Sprite(pygame.sprite.Sprite):
    ...
    def change_color_and_move_back(self):
        self.rect.x = max(min(self.rect.x - 40, SCREEN_WIDTH - self.rect.width), 0)
        self.rect.y = max(min(self.rect.y - 40, SCREEN_HEIGHT - self.rect.height), 0)
...
while running:
    ...
    if not won:
        ...
        if sprite1.rect.colliderect(sprite2.rect):
            sprite1.change_color_and_move_back()
            sprite2.change_color_and_move_back()
            won = True
    ...
    if won:
        win_text = font.render("DA ENDDDDDD!!!!", True, pygame.Color('black'))
